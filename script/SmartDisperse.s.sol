...
contract DeployAndTransfer is Script {
    ...
    function logBalances(uint256 chainId, ISuperchainWETH token) internal view {
        ...
        for (uint i = 0; i < recipients.length; i++) {
            console2.log(token.balanceOf(recipients[i]));
        }
        console2.log("----------------------------------------\n");
    }
    ...
    function run() external {
        ...
        vm.stopBroadcast();
        console2.log(
            "SmartDisperse deployed on Chain 901 at:",
            address(disperse901)
        );
        ...
        console2.log(
            "SmartDisperse deployed on Chain 902 at:",
            address(disperse902)
        );
        vm.stopBroadcast();
        ...
        console2.log(
            "Balance of contract 902 :",
            token901.balanceOf(address(disperse901))
        );
        logBalances(902, token902);
    }
}
